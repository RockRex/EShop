...
value="${productTypeBean}"></c:set> 
  ...
  var contentM = ""; 
  if ("${item0!=null}") { 
    contentM = "<li><a href='productTypeServlet?method=list&id="+"${item0.id}"+"'>"+"${item0.name}"+"</a></li>"+contentM; 
...
<body> 
  ...
  <td><a href="productTypeServlet?method=list&id=${item.id }">${item
.name}</a></td> 
          <td>${item.sort }</td> 
          <td>${item.intro }</td> 
          <td>${item.createDate }</td> 
       
  <td><a href="productTypeServlet?method=update&id=${item.id }">修改
</a></td> 
         <td><a href="productTypeServlet?method=delete&id=${item.id }">删除
</a></td> 
        </tr> 
      </c:forEach> 
    </table> 
